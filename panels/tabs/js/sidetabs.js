...
require(['js/tabs'], function(Tabs) {

  'use strict';

  let link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = 'style/sidetabs.css';
  ...
  function Tab(tabIframe) {
    ...
    button.onmouseup = (event) => {
      if (event.button == 0) {
        ...
        Tabs.remove(tabIframe);
      }
    };

    hbox.onmousedown = (event) => {
      if (event.button == 0) {
        Tabs.select(tabIframe);
      }
    };
  ...
  for (let tabIframe of Tabs) {
    let tab = new Tab(tabIframe);
    allTabs.set(tabIframe, tab);
  }

  let tabIframe = Tabs.getSelected();
  if (tabIframe) {
